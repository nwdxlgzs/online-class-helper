...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		render: function(ops) {
			this.count = ops.count || this.settings.count;//用户输入的数据总条数
			this.pagesize = ops.pagesize || this.settings.pagesize;
			this.current = parseInt(ops.current) || parseInt(this.settings.current);
			this.pagecount = Math.ceil(this.count / this.pagesize);//页码数=总条数/每页多少条
			this.format();
		},
		bindEvent: function() {
			var _this = this;
		...
		go: function(p) {
			...
			$(".publicHeader").css("position", "static");
		},
		changePagesize: function (ps) {
		    window.sessionStorage.setItem("cloudSystemNowPageSize", ps);//记录页面当前的页数

			this.render({
				pagesize: ps
			});
			this.settings.callback && this.settings.callback(1, this.pagesize, ps);
			var wrapHeight = $(".left_tree_wrap").parent().height();
			$(".publicHeader").css("position", "static");
		},
		format: function() {
			var html = '<ul>'
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><select class="ui-select-pagesize"></select><input type="text" class="ui-paging-count"/><a href="javascript:void(0)">跳转</a><li class="ui-pager-total">共计：'+this.count+'条</li>');
