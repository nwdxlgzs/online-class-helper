...
window.onload = function () {
    auto_click();
    //debug.success("函数正在执行，30秒一次！由于采用注入JS，debug.js不能完整运行，而且为了调起你所看见的这个控制台，模拟按了F2来启动这个页面，但是这个函数也有问题…控制台会拦截原生控制台消息，脚本错误也有可能是直播课堂那一方的…但是放心，模拟点击函数没有错误哦（当有运行中的提示时有签到弹窗，程序没有点击，而是报错才是出错（注意时间戳）。可以等30秒看看是不是还在打印运行中确定函数是不是还在运行。）。");
    log.w("函数正在执行，30秒一次！由于采用注入JS，debug.js不能完整运行，而且为了调起你所看见的这个控制台，模拟按了F2来启动这个页面，但是这个函数也有问题…控制台会拦截原生控制台消息，脚本错误也有可能是直播课堂那一方的…但是放心，模拟点击函数没有错误哦（当有运行中的提示时有签到弹窗，程序没有点击，而是报错才是出错（注意时间戳）。可以等30秒看看是不是还在打印运行中确定函数是不是还在运行。）。");
    fireKeyEvent(document.getElementsByTagName("head"), 'keydown', 113);  
};
...
//这个是debug.js，是以控制台样式显示的，比较适合电脑测试
